	...
	const std::string& tooltip_)
{
	...
	overlay_ob->material.albedo_texture = opengl_engine->getTexture(tex_path); // NOTE: currently we don't support mip-maps for uncompressed textures, which we need.  Turn compression off once this is supported.
