...
void AsyncGeometryUploader::checkForUploadedGeometry(OpenGLEngine* opengl_engine, uint64 frame_num, js::Vector<AsyncUploadedGeometryInfo, 16>& uploaded_geom_out)
{
	...
	while(!uploading_geometry.empty())
	{
		...
		{
			// The front item in queue, which is also the oldest, was created this frame, so don't check it or any of the others yet.
			break;
		}
	}
