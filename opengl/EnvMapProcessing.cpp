		...
		for(int theta_i=0; theta_i<100; ++theta_i)
		{
			...
			if(true)
			{
				...
				for(int face=0; face<6; ++face)
				{
					...
					EXRDecoder::saveImageToEXR(*diffuse_map, "diffuse_sky_no_sun_" + toString(theta_i) + "_" + toString(face) + ".exr", /*layer_name=*/"diffuse", options);
				} // End for each face
			}
			...
			if(true)
			{
				...
				EXRDecoder::SaveOptions options;
				options.compression_method = EXRDecoder::CompressionMethod_DWAB;
				EXRDecoder::saveImageToEXR(*combined_map, "specular_refl_sky_no_sun_combined_" +  toString(theta_i) + ".exr", /*layer name=*/"diffuse", options);
			}
