...
public:
	...
	virtual size_t getMapHeight() const { assert(0); return 1; }
	virtual size_t numChannels() const { assert(0); return 1; }

	virtual bool takesOnlyUnitIntervalValues() const { assert(0); return true; }

	virtual Reference<Map2D> extractChannelZero() const { assert(0); return NULL; }

	virtual Reference<ImageMap<float, FloatComponentValueTraits> > extractChannelZeroLinear() const { assert(0); return NULL; }

	virtual Reference<Image> convertToImage() const { assert(0); return NULL; }
	...
	virtual Reference<Map2D> getBlurredLinearGreyScaleImage(glare::TaskManager& task_manager) const { assert(0); return NULL; }
	...
	virtual Reference<ImageMap<float, FloatComponentValueTraits> > resizeToImageMapFloat(const int width, bool& is_linear) const { assert(0); return NULL; }
	...
	virtual size_t getByteSize() const { assert(0); return 1; }; // Get total size of image in bytes.  Returns the compressed size if the image is compressed.

	virtual float getGamma() const { assert(0); return 1; };
