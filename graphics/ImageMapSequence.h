...
public:
	...
	virtual inline Reference<Map2D> resizeMidQuality(const int new_width, const int new_height, glare::TaskManager* task_manager) const override;
#endif
...
inline Reference<Map2D> ImageMapSequence<V, ComponentValueTraits>::resizeMidQuality(const int new_width, const int new_height, glare::TaskManager* task_manager) const
{
	...
	for(size_t i=0; i<images.size(); ++i)
		new_seq->images[i] = this->images[i]->resizeMidQuality(new_width, new_height, task_manager).template downcast<ImageMap<V, ComponentValueTraits>>();
