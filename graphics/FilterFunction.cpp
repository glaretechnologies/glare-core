...
// The above function returns a non-const pointer to internal data, which is quite unsafe.
// This function returns a copy of the data.  TODO: factor out common code.
const std::vector<float> FilterFunction::getFilterDataVec(int supersample_factor) const
{
	std::vector<float> new_filter_data;

	const double filter_pixel_span_f = supportRadius() * 2 * supersample_factor;
	const int filter_pixel_span  = (int)ceil(supportRadius() * 2) * supersample_factor;
	const int filter_pixel_bound = filter_pixel_span / 2 - 1;

	new_filter_data.resize((filter_pixel_span - 1) * (filter_pixel_span - 1));
	...
	for(int u = -filter_pixel_bound; u <= filter_pixel_bound; ++u)
	{
		...
		new_filter_data[filter_addr++] = (float)(eval(r) / filter_sum);
	}
