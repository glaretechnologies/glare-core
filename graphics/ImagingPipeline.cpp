	...
	)
{
	...
	if(renderer_settings.lightMapBakingEnabled())
	{
		...
		for(int i=0; i<12; ++i) // The number of iterations gives the effective distance/radius of the flood fill.  More leaves fewer gaps but is slower to compute.
		{
			scratch_state.pre_filled_lightmap = ldr_buffer_out; // Copy the buffer, we don't want to modify it while reading from it.
			...
			for(int y=0; y<yres; ++y)
				for(int x=0; x<xres; ++x)
				{
					...
					if(alpha < alpha_threshold)
					{
						...
						{
							closest_src_col *= (1 / closest_src_col.x[3]); // Adjust for partial coverage  NOTE: shouldn't be needed.
		...
		for(int y=0; y<yres; ++y)
			for(int x=0; x<xres; ++x)
			{
				// Set unfilled areas to a default colour.
				//const float alpha = ldr_buffer_out.getPixel(x, y).x[3];
				//if(alpha < alpha_threshold)
				//{
