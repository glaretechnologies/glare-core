...
public:
	...
	struct SizeInfo
	{
		...
		Vec2i max_bounds; // Maximum pixel coordinates of glyph rectangle, with x increasing to right, y increasing up.
	...
	// font must be non-null.  emoji_font can be null, in which case emoji characters will be drawn with a box.
	// Throws glare::Exception on failure, for example on invalid UTF-8 string.
	SizeInfo getTextSize(const string_view text, TextRendererFontFace* font, TextRendererFontFace* emoji_font); 
	...
	// font must be non-null.  emoji_font can be null, in which case emoji characters will be drawn with a box.
	// Throws glare::Exception on failure, for example on invalid UTF-8 string.
	void drawText(ImageMapUInt8& map, const string_view text, int x, int y, const Colour3f& col, bool render_SDF, TextRendererFontFace* font, TextRendererFontFace* emoji_font);
...
public:
	...
	// Throws glare::Exception on failure, for example on invalid UTF-8 string.
	TextRenderer::SizeInfo getGlyphSize(const string_view char_text, bool render_SDF);

	// Get size info for a string that may have multiple characters.
	// NOTE: just about as slow as drawText().
	// Assumes not doing SDF rendering
	// Throws glare::Exception on failure, for example on invalid UTF-8 string.
	TextRenderer::SizeInfo getTextSize(const string_view text);
