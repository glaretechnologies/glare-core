...
private:
	...
	// We need to object arrays, so we can do a parallel partition that isn't in-place.
	js::Vector<BinningOb, 64> objects_a;
	js::Vector<BinningOb, 64> objects_b;
...
void BinningBVHBuilder::setObjectAABB(size_t ob_i, const js::AABBox& aabb)
{
	...
	objects_a[ob_i].aabb = aabb;
	objects_a[ob_i].setIndex((int)ob_i);
}
