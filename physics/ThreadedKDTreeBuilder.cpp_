...
=====================================================================*/
#include "ThreadedKDTreeBuilder.h"

#include <algorithm>
#include "../graphics/TriBoxIntersection.h"
#include "../utils/poolworkerthread.h"
...
ThreadedKDTreeBuilder::ThreadedKDTreeBuilder()
{
...
class BuildSubTreeWorker : public PoolWorkerThread<ThreadedKDTreeBuilder::BuildTask>
{
public:
	BuildSubTreeWorker(ThreadPool<ThreadedKDTreeBuilder::BuildTask>* pool)
		:	PoolWorkerThread(pool)
	{}
	~BuildSubTreeWorker(){}


	virtual void doHandleTask(ThreadedKDTreeBuilder::BuildTask& task)
	{
		// The task describes a particular subtree that needs to be build
		ThreadedKDTreeBuilder::IntermediateNode* node = task.node;

		task.builder->doBuild(
			*node->tree,
			0,
			*node->tris,
			node->depth,
			node->maxdepth,
			node->aabb,
			node->leaf_tri_indices_out,
			node->nodes_out
			);

		// Cleanup memory allocated for the task
		delete node->tris;

	
		/*task.builder->buildSubtree(
		...
		{
			Lock lock(task.builder->mutex);

			task.builder->num_subtrees_left_to_build--;
			assert(task.builder->num_subtrees_left_to_build >= 0);

			if(task.builder->num_subtrees_left_to_build == 0)
				task.builder->build_threads_finished.notify();
		}
	}

	virtual bool isTaskFinishedSentinel(const ThreadedKDTreeBuilder::BuildTask& task)
	{
		return task.node == NULL;
	}
};
...
buildSubtree
	...
	if num tris < threshold && num queued work items < some threshold
...
void ThreadedKDTreeBuilder::build(TriTree& tree, const AABBox& root_aabb, TriTree::NODE_VECTOR_TYPE& nodes_out, js::Vector<TriTree::TRI_INDEX>& leaf_tri_indices_out)
{
	...
	{ // Scope for various arrays
		...
		std::vector<TriInfo>* tris = new std::vector<TriInfo>;

		//for(int t=0; t<MAX_KDTREE_DEPTH; ++t)
		//	node_tri_layers[t].reserve(1000);
		
		tris->resize(tree.numTris());
		for(unsigned int i=0; i<tree.numTris(); ++i)
		{
			...
			(*tris)[i].lower = tri_aabb.min_;
			(*tris)[i].upper = tri_aabb.max_;
		}
		...
		// Add initial task to threadpool
		root_intermediate_node = new IntermediateNode;
		root_intermediate_node->left = root_intermediate_node->right = root_intermediate_node->parent = NULL;
		root_intermediate_node->tris = tris;

		BuildTask task;
		task.node = root_intermediate_node;
		task.builder = this;
		num_subtrees_left_to_build = 1;

		const int num_threads = 3;
		spawnThreads<BuildSubTreeWorker, BuildTask>(thread_pool, num_threads);
		thread_pool.enqueueTask(task);

		this->build_threads_finished.wait(this->mutex);

		// Insert 'task finished' tasks into queue
		BuildTask finished_task;
		finished_task.node = NULL;
		finished_task.builder = NULL;
		thread_pool.insertFinishedTasks(finished_task);
		thread_pool.waitForThreadTermination();
	}
}
...
static inline bool SortedBoundInfoLowerPred(const ThreadedKDTreeBuilder::SortedBoundInfo& a, const ThreadedKDTreeBuilder::SortedBoundInfo& b)
{
   return a.lower < b.lower;
}

static inline bool SortedBoundInfoUpperPred(const ThreadedKDTreeBuilder::SortedBoundInfo& a, const ThreadedKDTreeBuilder::SortedBoundInfo& b)
{
...
void ThreadedKDTreeBuilder::buildSubtree(
...
void ThreadedKDTreeBuilder::doBuild(IntermediateNode* node, const TriTree& tree, unsigned int cur, //index of current node getting built
						const std::vector<TriInfo>& nodetris,
...
//	std::vector<TriInfo>& child_tris = node_tri_layers[depth + 1];
	...
	std::vector<TriInfo>* left_child_tris = new std::vector<TriInfo>;

	//------------------------------------------------------------------------
	// Build list of neg tris
	//------------------------------------------------------------------------
	left_child_tris->resize(0);
	left_child_tris->reserve(myMax(best_num_in_neg, best_num_in_pos));
		...
		if(tri_lower <= split) // Tri touches left volume, including splitting plane
		{
			if(tri_lower < split) // Tri touches left volume, not including splitting plane
			{
				if(tri_upper > split)
				{
					...
					left_child_tris->push_back(TriInfo());
					left_child_tris->back().tri_index = nodetris[i].tri_index;
					...
					left_child_tris->back().lower = clipped_tri_aabb.min_;
					left_child_tris->back().upper = clipped_tri_aabb.max_;
#else
					left_child_tris->push_back(nodetris[i]);
#endif
				}
				else // else tri_upper <= split
					left_child_tris->push_back(nodetris[i]); // Tri is only in left child
			}
			else
			{
	...
	assert(left_child_tris->size() == best_num_in_neg);

	//------------------------------------------------------------------------
	//create negative child node, next in the array.
	//------------------------------------------------------------------------
	const unsigned int actual_num_neg_tris = (unsigned int)left_child_tris->size();
