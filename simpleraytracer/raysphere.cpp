...
void RaySphere::getInfoForHit(const HitInfo& hitinfo, Vec3Type& N_g_os_out, Vec3Type& N_s_os_out, unsigned int& mat_index_out, Vec3Type& pos_os_out, Real& pos_os_rel_error_out, Real& curvature_out) const
{
	...
	pos_os_rel_error_out = std::numeric_limits<Real>::epsilon();
	curvature_out = recip_radius; // Mean curvature for a sphere is just the reciprocal radius of the sphere.
}
...
bool RaySphere::subdivideAndDisplace(ThreadContext& context, const Object& object, const Matrix4f& object_to_camera, /*const CoordFramed& camera_coordframe_os, */ double pixel_height_at_dist_one,
